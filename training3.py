...
def train(src, tgt, line_file, evaluation_split="validation"):
    ...
    """
    split_datasets = nllb_data(src, tgt, lines)
    ...
    def preprocess_function(examples):
        ...
        targets = [ex[src] for ex in examples["translation"]]
    ...
    def compute_metrics(eval_preds):
        ...
        result2 = metric2.compute(predictions=decoded_preds, references=decoded_labels, char_order=6)
        return {"bleu": result1["score"], "chrf": result2["score"]}
    ...
    with open(f"chrf.{evaluation_split}.{model_dir}", "w") as writer:
        writer.write(f"{results['eval_chrf']}\n")
